...
| macOS | SwiftUI application targeting macOS 26.5, with an app icon, menu-bar status item, and floating Agent that opens its own compact chat UI. Firebase initializes from the shared iOS Apple app (`com.Ficruty.caocap`). The status menu can Sign in with Apple and restore that Firebase UID; device linking and remote commands are not implemented. Prompts are session-only and are not sent to an agent service; computer use is not implemented. The hub window is still placeholder content. There is no test target. Setup is described in `REF-006`. |
