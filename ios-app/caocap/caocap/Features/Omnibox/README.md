...
- `CommandPaletteViewModel` owns query state, filtered actions, selected index, presentation state, and action emission.
...
5. The view model emits the selected `AppActionID` through `onExecute`.
6. `AppActionDispatcher.perform(_:source:)` validates source safety and runs the configured handler.
...
- Keep side effects out of `CommandPaletteViewModel`; emit IDs and let the dispatcher/app shell execute.
