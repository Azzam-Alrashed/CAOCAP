...
3. **Interactive tutorial** (`OnboardingCoordinator`) — opens through the Tutorial portal, then continues with gesture-driven tooltips on the live canvas (`onboarding_completed_v7`). Steps are grouped into five lessons (≤6 steps each) defined in `OnboardingLessonsManifest.swift`; each lesson has a unique accent color. The Canvas Navigation lesson starts in the Tutorial subcanvas with omnibox Go Back, then teaches pan, zoom, fit-all, omnibox fly-to, and portal entry on the root canvas. **Mini-App Preview** teaches opening the seeded Hello World node, interacting with the live preview, editing code, and returning to the canvas. **Move & Organize** teaches dragging the practice node, omnibox organize/grid commands, and undo/redo. Skipped users can relaunch individual lessons from Help → **Interactive lessons**.
